function varargout = createPolishingIP(input)

%% Check Inputs

%check for X
if isfield(input,'skip_checks') && input.skip_checks
    
    X               = input.X;
    Y               = input.Y;
    active_set      = input.active_set;
    coefCons        = input.coefConstraints;
    pos_indices     = Y==1;
    neg_indices     = ~pos_indices;
    N_o             = size(X,1);
    N_pos_o         = sum(pos_indices);
    N_neg_o         = N_o - N_pos_o;
    ...
    %check start lambda
    assert(isfield(input,'active_set'), 'input must contain a set of coefficients to run polishing')
    active_set = input.active_set(:);
    assert(size(active_set,1) == P)
...
%filter everything to active set
active_set      = active_set~=0;
active_set(1)   = 1;
coefCons        = coefCons.subset(active_set);
L0_max          = sum(active_set) - sum(coefCons.C_0j == 0);
L0_min          = L0_max;
[X, nY_pos, nY_neg, pos_loss_con, neg_loss_con, conflict_table] = compress_data(X(:,active_set),Y);
[N, P]           = size(X);
...
err_min        = max(sum(min_conflicts,1),err_min);
...
C_1                 = 0.975.*min(w_pos/N_o,w_neg/N_o)./L1_max;
C_1                 = C_1.*ones(P,1);
...
%variables  =                 lambda_pos (P) lambda_neg (P) alpha_pos(P)    alpha_neg(P)    pos_errors (nZ_pos)     negative errors (nZ_pos))
